...
class SynWorker(context: Context, params: WorkerParameters) : Worker(context, params) {
    ...
    override fun doWork(): Result {
        ...
        Log.d("Synworker", "${jsonString}")
        val lastSynTime = SharedPreferenceUtil.readString("lastSynTime")
        val url = "http://10.236.11.105:8080/portal/notes/syn.do?lastSynTime=${lastSynTime}"
        OKHttpUtils.post(url, jsonString, object : OKHttpCallback() {
            override fun onFinish(status: String, msg: String) {
                ...
                if (status.equals("success")) {
                    ...
                    val  asyn_list =NotesDatabase.getDatabase(MyApplication.context).asynDao().selectAll(userVo.userId!!)
        ...
        for (item in requestDatas) {
            ...
            if (item.desc.equals("request")){
                if (item.operation.equals("add")) {
                    ...
                    notes.id = null
                    Log.d("SynWorker", "服务端需要更新到本地的部分，Add...${notes.title}")
                    Log.d("SynWorker", "协程....")
                    var id =
                    ...
                    Log.d("Synworker", "要更新的notesid是${offline_Id}")
                    var notes =
                    ...
                    Log.d("SynWorker", "服务端需要更新到本地的部分，Update...${notes.title}")
                    NotesDatabase.getDatabase(MyApplication.context).noteDao().updateNote(notes)

                } else {

                    Log.d("SynWorker", "服务端需要更新到本地的部分，delete...${item.online_id}")
                ...
                if(item.operation.equals("add")){
                    ...
                    NotesDatabase.getDatabase(MyApplication.context).idmapDao().updateOnlineID(online_id,offline_id)
                    val userVo=SharedPreferenceUtil.readObject("user",UserVo::class.java)
                    val  idmap_list =NotesDatabase.getDatabase(MyApplication.context).idmapDao().selectAll(userVo.userId!!)
                    Log.d("Synworker","idmap长度 ${idmap_list.size}")
                    for( i in  idmap_list){
                        Log.d("Synworker","idmap offline_id:${i.offlineId}, online_id${i.onlineId}")
                    }
                }
            }
    ...
    fun getLocalChanges(): ArrayList<RequestData> {
        ...
        Log.d("SynWorker is working", "正在收集本地需要更改的内容")
        val userVo=SharedPreferenceUtil.readObject("user",UserVo::class.java)
        val asyns =
